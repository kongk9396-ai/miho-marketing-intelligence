...
import { runManualGa4SyncAction, testGa4ConnectionAction } from "@/app/data/ga4-sync/actions";
...
export default async function Ga4SyncPage() {
    ...
    if (err instanceof SchemaNotReadyError) {
      return (
        <>
          <PageHeader
            title="GA4 자동 수집"
            description="Google Analytics 4 데이터를 자동으로 수집하여 저장합니다."
          />
  ...
  const historyColumns: DataTableColumn<Ga4SyncHistoryRecord>[] = [
    ...
    { key: "sync_date", header: "수집 대상 날짜", render: (row) => row.sync_date ?? "—" },
    {
      key: "status",
      header: "상태",
      render: (row) => (
        <StatusBadge label={row.status === "success" ? "성공" : "실패"} variant={row.status === "success" ? "success" : "danger"} />
      ),
    },
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <ActionButton label="GA4 연결 테스트" icon={<PlugZap strokeWidth={2} />} action={testGa4ConnectionAction} />
            <ActionButton
              label="지금 동기화"
              pendingLabel="동기화 중..."
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <div className="rounded-lg border border-gray-200 bg-white p-5">
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium text-gray-500">GA4 연결 상태</p>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-5">
          <p className="text-sm font-medium text-gray-500">속성 ID</p>
          <p className="mt-2 text-sm font-semibold text-gray-900">{status.propertyId ?? "—"}</p>
        </div>

        <div className="rounded-lg border border-gray-200 bg-white p-5">
          <p className="text-sm font-medium text-gray-500">마지막 동기화</p>
          <p className="mt-2 text-sm font-semibold text-gray-900">{formatOrDash(status.lastSyncAt)}</p>
        </div>

        <div className="rounded-lg border border-gray-200 bg-white p-5">
          <p className="text-sm font-medium text-gray-500">최근 데이터 날짜</p>
          <p className="mt-2 text-sm font-semibold text-gray-900">{status.latestDataDate ?? "—"}</p>
        </div>

        <div className="rounded-lg border border-gray-200 bg-white p-5">
          <p className="text-sm font-medium text-gray-500">최근 7일 수집 행 수</p>
          <p className="mt-2 text-sm font-semibold text-gray-900">{status.recent7DayRowCount}행</p>
        </div>
