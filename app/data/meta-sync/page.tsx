...
import { getMetaSyncStatusView, type OverallSyncStatus } from "@/lib/meta/sync-status";
...
const STATUS_LABELS: Record<OverallSyncStatus, { label: string; variant: StatusVariant }> = {
  ok: { label: "정상", variant: "success" },
  connection_required: { label: "연결 필요", variant: "warning" },
  last_sync_failed: { label: "마지막 동기화 실패", variant: "danger" },
  no_new_reports: { label: "새로운 보고서 없음", variant: "info" },
  not_yet_run: { label: "아직 실행되지 않음", variant: "neutral" },
};
...
const IMPORT_STATUS_LABELS: Record<string, { label: string; variant: StatusVariant }> = {
  ...
  no_new_reports: { label: "새 보고서 없음", variant: "neutral" },
};
...
export default async function MetaSyncPage({ searchParams }: MetaSyncPageProps) {
  const params = await searchParams;

  let status;
  let settings;
  try {
    [status, settings] = await Promise.all([getMetaSyncStatusView(), getMetaSyncSettings()]);
  } catch (err) {
    if (err instanceof SchemaNotReadyError) {
      return (
        <>
          <PageHeader
            title="Meta 자동 수집"
            description="Meta Ads Manager 예약 보고서 이메일을 자동으로 수집하여 저장합니다."
          />
  ...
  const historyColumns: DataTableColumn<MetaImportHistoryRecord>[] = [
    ...
    { key: "source_type", header: "출처", render: (row) => SOURCE_LABELS[row.source_type] ?? row.source_type },
    ...
    { key: "row_count", header: "행 수", align: "right" },
    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <SyncStatusCard
          ...
          latestDataDateLabel={status.latestDataDate ?? "—"}
        />

        <div className="rounded-lg border border-gray-200 bg-white p-5">
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium text-gray-500">Gmail 연결 상태</p>
            <StatusBadge
              label={status.gmailConnected ? "연결됨" : "연결 필요"}
              variant={status.gmailConnected ? "success" : "warning"}
            />
          </div>
          <p className="mt-2 text-base font-semibold text-gray-900">
            {status.gmailConnected ? (status.gmailEmail ?? "연결된 계정") : "Gmail 연결이 필요합니다."}
          </p>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-5">
          <p className="text-sm font-medium text-gray-500">오늘 수집 건수</p>
          <p className="mt-2 text-2xl font-semibold text-gray-900">{status.todayCount}건</p>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-5">
          ...
          <p className="mt-2 text-sm font-semibold text-gray-900">{formatOrDash(status.lastSuccessAt)}</p>
        </div>
        <div className="rounded-lg border border-gray-200 bg-white p-5">
          <p className="text-sm font-medium text-gray-500">마지막 처리 파일</p>
          <p className="mt-2 truncate text-sm font-semibold text-gray-900">{status.lastFile ?? "—"}</p>
        </div>
        <div className="rounded-lg border border-gray-200 bg-white p-5">
      ...
      <div className="mt-4 rounded-md border border-gray-200 bg-white px-4 py-3 text-sm text-gray-600">
        자동 수집에 문제가 있을 경우{" "}
        <Link href="/data/meta-csv-upload" className="font-medium text-blue-600 hover:underline">
          Meta CSV 수동 업로드
        </Link>
        를 사용할 수 있습니다.
      </div>
      ...
      <div className="mt-6">
        <DataTable
          ...
          emptyMessage="아직 처리된 내역이 없습니다."
        />
      </div>

      <div id="collection-settings" className="mt-6 rounded-lg border border-gray-200 bg-white p-5">
        <h3 className="text-sm font-semibold text-gray-900">수집 설정</h3>
        <p className="mt-1 text-xs text-gray-500">
          제목 키워드, 검색 기간, 첨부파일 형식, 자동 수집 여부를 설정합니다.
        </p>
        <div className="mt-4">
