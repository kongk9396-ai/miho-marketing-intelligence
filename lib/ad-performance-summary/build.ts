...
async function buildCreativeChangeSection(): Promise<CreativeChangeSection> {
  ...
  const [dbBeforeRows, dbAfterRows] = await Promise.all([
    getLeadsForCampaignContent(resolved.utmCampaign, resolved.utmContent, kstDateOnlyToInstantIso(periods.before.start), kstDateOnlyToInstantIso(addDaysToDateOnly(periods.before.end, 1))),
    getLeadsForCampaignContent(resolved.utmCampaign, resolved.utmContent, kstDateOnlyToInstantIso(periods.after.start), kstDateOnlyToInstantIso(addDaysToDateOnly(periods.after.end, 1))),
  ]);
