...
const ATTRIBUTION_HEADER_ALIASES: Record<string, string[]> = {
  ...
  submitted_at: ["submitted_at", "신청일", "신청날짜", "신청일시"],
...
/** Exact timestamp fallback for historical attribution rows that have no source_sheet/source_row. */
export function attributionSubmittedAtKey(submittedAtIso: string): string {
  return `submitted_at|||${submittedAtIso}`;
}
...
export function parseAttributionRecords(records: Record<string, unknown>[]): AttributionRecord[] {
  ...
  if ((!headerMap.source_sheet || !headerMap.source_row) && !headerMap.submitted_at) return [];
  ...
  return records
    .map((record) => {
      ...
      const parsedSourceRow = sourceRowRaw !== null ? Number(sourceRowRaw) : NaN;
      const sourceRow = Number.isFinite(parsedSourceRow) ? parsedSourceRow : null;
      const submittedAt = parseSheetDateTime(get("submitted_at"));

      // New records can use the stable source_sheet/source_row join.
      // Historical DBcart rows without those columns may fall back to an
      // exact submitted_at match. Rows with neither identity are ignored.
      if ((!sourceSheet || sourceRow === null) && !submittedAt) return null;
...
export async function fetchAttributionMatchMap(
  ...
  // Timestamp fallback is allowed only when that timestamp is unique.
  // Ambiguous timestamps are deliberately left unmatched rather than
  // attributing a lead to the wrong ad.
  const submittedAtCounts = new Map<string, number>();
  for (const record of parsed) {
    if (!record.submittedAt) continue;
    submittedAtCounts.set(
      record.submittedAt,
      (submittedAtCounts.get(record.submittedAt) ?? 0) + 1
    );
  }

  for (const record of parsed) {
    if (record.sourceSheet && record.sourceRow !== null) {
      map.set(
        attributionMatchKey(record.sourceSheet, record.sourceRow),
        record
      );
    }
    ...
    if (
      record.submittedAt &&
      submittedAtCounts.get(record.submittedAt) === 1
    ) {
      map.set(attributionSubmittedAtKey(record.submittedAt), record);
    }
  }
